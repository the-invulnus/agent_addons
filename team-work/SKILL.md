...
每位团队成员都拥有完全独立的工作空间，git仓库将作为团队成员之间共享文件的唯一途径。使用git工具用于集中的文件共享和版本管理。如果用户发布的任务需要用到git工具，但是用户没有提供git仓库地址，团队lead需要向用户索要git仓库地址，并明确告知团队成员，开发所使用的**仓库地址**和**开发分支**。在向其他成员发送消息之前（分配任务或汇报结果），请先使用git工具将本地修改提交到git远端仓库，再发送消息，确保文件视图的一致性。

### Git权限认证
使用环境变量中的`GIT_USER`和`GIT_ACCESS_TOKEN`进行git权限认证。例如：
```
git clone https://${GIT_USER}:${GIT_ACCESS_TOKEN}@git.shinemo.com/scm/path/to/repo.git
```
优先直接引用环境变量的值进行访问，不要打印出环境变量的真实值。如果git权限认证失败，再确认环境变量中是否存在`GIT_USER`和`GIT_ACCESS_TOKEN`，如果没有，可直接向用户索要。
