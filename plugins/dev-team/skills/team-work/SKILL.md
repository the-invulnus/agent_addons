...
每位团队成员都拥有完全独立的工作空间，Git仓库将作为团队成员之间共享文件的唯一途径。使用Git工具用于集中的文件共享和版本管理。
- 当用户配置了Git仓库，则使用用户配置的仓库地址和开发分支，如遇到认证问题，由团队lead统一向用户寻求授权方式，并向团队成员同步。
- 当用户未配置Git仓库，由团队lead新建本地Git bare仓库，并告知团队成员Git仓库地址和开发分支，每位成员在**各自的本地工作空间**中clone该仓库，并提交到Git远端仓库。

将Git仓库的初始化作为团队协作的第一步，确保成员都完成了Git仓库的初始化，再开始后续的协作任务。团队成员在协作过程中，如果对Git仓库中的文件进行了增删改，在向其他成员发送消息之前（分配任务或汇报结果），请先使用Git工具push本地修改，再发送消息，确保其他成员对于文件视图的一致性。

### Git访问授权
当用户配置了Git仓库时，根据配置中的`GIT_AUTH_TYPE`来判断是否需要进行额外的访问授权。如果`GIT_AUTH_TYPE`为`https`，则使用环境变量或上下文中的`GIT_USER`和`GIT_ACCESS_TOKEN`进行git授权，例如：
```bash
...
当前团队正在执行软件开发项目，根据用户的要求，在软件架构设计上，必须采用以下配置：
- 数据库：SQLite
